...
def process_users(dataset, filename, b):
    id = []
    label = []
    tweets = []
    friends = []
...
def test():
    num0, id0, label0, tweets0, friends0, favourites0 = process_users('cresci-2017', ['genuine_accounts'],0) #human
    num1, id1, label1, tweets1, friends1, favourites1 = process_users('cresci-2017', ['social_spambots_1','social_spambots_2','social_spambots_3','fake_followers','traditional_spambots_1'],1) #bot
    # 合并人类账户和机器账户
    num = num0 + num1
    ...
    print('num:{},bot:{},human:{}'.format(num,np.sum(label_block),num-np.sum(label_block)))
    # 构图
    ...
    mask_favo = (diff_favo < (max_favo * 0.1))
    graph[2][mask_favo] = 1
    ...
    for i in comms.keys():
        ...
        if n_beta >= 1:  #botwiki:0.55; pronbots:0.6; cresci:1
            num_bot_comm += 1
            for node in comm:
                pre_bot[node] = 1
        else:
